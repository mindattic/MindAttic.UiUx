...
#   - Cyberspace/         (BEGIN/END MINDATTIC.CONTENT:CYBERSPACE)
#       Cyberpunk console-background effects: scan lines, fake error
#       popups, floating fragments, network attempts, memos, etc.
#       Files: Cyberspace/frontpage.html, Cyberspace/frontpage.css,
#              Cyberspace/loader.js, Cyberspace/tv-static.js, Cyberspace/home-bg.js,
#              Cyberspace/console-bg.js
#   - PinFooter/  (BEGIN/END MINDATTIC.CONTENT:PINFOOTER)
#       Pin a `.pin-when-short` element to the viewport bottom when
#       the page is shorter than the viewport.
#       Files: PinFooter/pin-footer.html, PinFooter/pin-footer.css,
#              PinFooter/pin-footer.js
#
...
# --------------------------------------------------------------------
# Build the CYBERSPACE block
# --------------------------------------------------------------------
function Build-CyberspaceBlock {
    param([string]$ContentRoot)

    $cyberspace = Join-Path $ContentRoot 'Cyberspace'
    if (-not (Test-Path $cyberspace)) { throw "Cyberspace/ not found at $cyberspace" }

    $html = [System.IO.File]::ReadAllText((Join-Path $cyberspace 'frontpage.html'), $utf8)
    ...
    # reads window.__cyberspaceCircuitboardSrcs if defined; StreetSamurai's MediaController
    ...
    $texOverride = ''
    $texDir = Join-Path $cyberspace 'assets'
    if (Test-Path $texDir) {
    ...
    $jsConcat = $texOverride + "`r`n" + (($jsParts | ForEach-Object {
        "/* ---- $_ ---- */`r`n" + [System.IO.File]::ReadAllText((Join-Path $cyberspace $_), $utf8)
    }) -join "`r`n`r`n")

    $block = "<!-- Generated by MindAttic.Content/sync/sync-mindattic-com.ps1. Do not edit by hand." + "`r`n" +
             "     Edit MindAttic.Content/Cyberspace/* and re-run the sync. -->" + "`r`n" +
             $html + "`r`n" +
             '<style id="mindattic-content-cyberspace-css">' + "`r`n" +
             $css + "`r`n" +
             '</style>' + "`r`n" +
             '<script id="mindattic-content-cyberspace-js">' + "`r`n" +
             $jsConcat + "`r`n" +
             '</script>'
...
# --------------------------------------------------------------------
# Build the PINFOOTER block
# --------------------------------------------------------------------
function Build-PinFooterBlock {
    param([string]$ContentRoot)

    $pf = Join-Path $ContentRoot 'PinFooter'
    if (-not (Test-Path $pf)) { throw "PinFooter/ not found at $pf" }
    ...
    $block = "<!-- Generated by MindAttic.Content/sync/sync-mindattic-com.ps1. Do not edit by hand." + "`r`n" +
             "     Edit MindAttic.Content/PinFooter/* and re-run the sync. -->" + "`r`n" +
             $html + "`r`n" +
...
# --------------------------------------------------------------------
$cyberspaceInfo = Build-CyberspaceBlock -ContentRoot $ContentRoot
...
$indexText = Splice-MarkerBlock -IndexText $indexText `
    -BeginMarker '<!-- BEGIN MINDATTIC.CONTENT:CYBERSPACE -->' `
    -EndMarker   '<!-- END MINDATTIC.CONTENT:CYBERSPACE -->' `
    -Block       $cyberspaceInfo.Block
$indexText = Splice-MarkerBlock -IndexText $indexText `
    -BeginMarker '<!-- BEGIN MINDATTIC.CONTENT:PINFOOTER -->' `
    -EndMarker   '<!-- END MINDATTIC.CONTENT:PINFOOTER -->' `
    -Block       $pfInfo.Block
...
Write-Output ("  CYBERSPACE:        html $($cyberspaceInfo.HtmlKb) KB, css $($cyberspaceInfo.CssKb) KB, $($cyberspaceInfo.JsCount) JS files $($cyberspaceInfo.JsKb) KB")
Write-Output ("  PINFOOTER: html $($pfInfo.HtmlKb) KB, css $($pfInfo.CssKb) KB, js $($pfInfo.JsKb) KB")
